    ...
    public function index(): void
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $values = [
                ...
                'reset_pin' => trim((string) ($_POST['reset_pin'] ?? '')),
                'reset_pin_confirm' => trim((string) ($_POST['reset_pin_confirm'] ?? '')),
            ...
            if (!verify_csrf_or_same_origin($_POST['csrf_token'] ?? null)) {
                ...
                $resetPin = $values['reset_pin'];
                $resetPinConfirm = $values['reset_pin_confirm'];
                if (strlen($password) < MIN_PASSWORD_LENGTH) {
                    $error = 'Password is too short. Use at least ' . MIN_PASSWORD_LENGTH . ' characters.';
                } elseif ($password !== $confirm) {
                    $error = 'Passwords do not match.';
                } elseif (!preg_match('/^\d{4}$/', $resetPin)) {
                    $error = 'Reset PIN must be exactly 4 digits.';
                } elseif ($resetPin !== $resetPinConfirm) {
                    $error = 'Reset PIN values do not match.';
